...
class _OrderDetailsState extends State<OrderDetails> {
  ...
  Future<List> getVouchers() async {
    ...
    var orderID = widget.id.toString();
    log(userId);
    log(orderID);
    Uri url2 = Uri.parse(
      "${AppConfig.BASE_URL}/user/voucher",
    );
    final response2 = await http.get(
      ...
      headers: {
        ...
        // 'Authorization': "Bearer 11|Fkzrakx9vlCJIytR5klCJqLsJETCdUPwQEKtoPYx",
      },
    );
    log("label 1 url");
    final map = await jsonDecode(response2.body.toString());
    log("label 2 url");
    ...
    for (var element in data) {
      ...
      final e1 = element["buyer_id"];
      final e2 = element["order_id"];
      if (userId.toString().compareTo(e1.toString()) == 0 &&
          orderID.compareTo(e2.toString()) == 0) {
        final map = Map.from(element);
        Uri url = Uri.parse(
          "${AppConfig.BASE_URL}/products/${map["product_id"]}",
        );
        ...
        // log(map["data"].runtimeType.toString());
        final data2 = map2["data"][0];
        // log(data2.toString());

        final productName = data2["name"];
